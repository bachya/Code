...
xmlns:ext="http://exslt.org/common" xmlns:myHalve="myHalve"
xmlns:myTenXController="myTenXController"
xmlns:myEasyDiffMap="myEasyDiffMap"
xmlns:myWithinController="myWithinController"
exclude-result-prefixes="xsl ext myHalve myTenXController myEasyDiffMap myWithinController">
  ...
  <xsl:template name="easyDiffList">
    ...
    <xsl:variable name="vmyTenXController"
    select="document('')/*/myTenXController:*[1]" />
    ...
    select="ext:node-set($vrtfMapParams)/*" />
  ...
  match="*[namespace-uri()='myWithinController']">
    ...
    <xsl:choose>
      <xsl:when test="not($vLastDiff &lt; $pParams and $vLastDiff &gt; (0 - $pParams)) and count($pList) &lt;= 40">
      1</xsl:when>
    </xsl:choose>
  </xsl:template>
