...
xmlns:ext="http://exslt.org/common" xmlns:pGenerator="pGenerator"
...
exclude-result-prefixes="xsl pGenerator pController pTakeController pTakeController2 pTakeController3">
  ...
  select="document('')/*/pTakeController:*[1]" />
  <xsl:variable name="vMyTakeController2"
  select="document('')/*/pTakeController2:*[1]" />
  <xsl:variable name="vMyTakeController3"
  select="document('')/*/pTakeController3:*[1]" />
  <xsl:template match="/">buildListWhile Generator: 2 * N,
  Controller: count &lt;= 10 
  ...
  <xsl:variable name="vList-1-10"
  select="ext:node-set($v1-10)/*" />
...
</xsl:text>takeWhile Generator: 2 * N, Controller: &lt; 7 
  <xsl:call-template name="takeWhile">
    <xsl:with-param name="pGenerator" select="$vMyGenerator" />
    <xsl:with-param name="pController"
    select="$vMyTakeController" />
  </xsl:call-template>
  <xsl:text>
...
</xsl:text>takeWhile Generator: 2 * N, Controller: count &lt;= 5 
  <xsl:call-template name="takeWhile">
    <xsl:with-param name="pGenerator" select="$vMyGenerator" />
    <xsl:with-param name="pController"
    select="$vMyTakeController3" />
  </xsl:call-template>
  <xsl:text>
...
</xsl:text>takeWhile Generator: 2 * N, N=8 
