...
exclude-result-prefixes="xsl ext myHalve myTenXController myEasyDiffMap myFunction myWithinController">
  ...
  match="*[namespace-uri()='myWithinController']">
    ...
    <xsl:choose>
      <xsl:when test="not($vLastDiff &lt; $pParams and $vLastDiff &gt; (0 - $pParams))">
      1</xsl:when>
    </xsl:choose>
  </xsl:template>
