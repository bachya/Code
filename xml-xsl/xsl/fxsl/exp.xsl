...
xmlns:fScaleByE="f:fScaleByE" xmlns:x="f:x-exp.xsl"
exclude-result-prefixes="xsl x ext fCondInInterval fScaleByE">
  ...
  <xsl:template name="ln">
    ...
    <xsl:if test="not($pX &gt; 0)">
      <xsl:message terminate="yes">
        <xsl:value-of select="concat('[Error]ln: non-positive argument passed:', $pX)" />
      </xsl:message>
    </xsl:if>
    ...
    <xsl:variable name="vScaleByE" select="$x:st/fScaleByE:*[1]" />
  ...
  <xsl:template name="log">
    ...
    <xsl:if test="not($pBase &gt; 0 and $pBase != 1)">
      <xsl:message terminate="yes">
        <xsl:value-of select="concat('[Error]log: Invalid log base: ', $pBase )" />
      </xsl:message>
    </xsl:if>
  ...
  <xsl:template name="pow">
    ...
    <xsl:if test="not($pBase &gt; 0)">
      <xsl:message terminate="yes">
        <xsl:value-of select="concat('[Error]pow: Non-positive pow base: ', $pBase )" />
      </xsl:message>
    </xsl:if>
    <xsl:variable name="vLogBase">
      <xsl:call-template name="ln">
  ...
  <xsl:template match="fScaleByE:*">
    ...
    select="number($arg1[name()='cnt'])" />
    <xsl:variable name="vX" select="number($arg1[name()='x'])" />
    <xsl:choose>
      <xsl:when test="$vX &gt; 1.5">
        <cnt>
          <xsl:value-of select="$vCnt + 1" />
        </cnt>
        <x>
          <xsl:value-of select="$vX div $vE" />
        </x>
      </xsl:when>
      <xsl:otherwise>
        <cnt>
