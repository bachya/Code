...
exclude-result-prefixes="xsl x ext injectValues extendAllLists appendToList">
...
-->
    ...
    <xsl:variable name="vInjectValues"
    select="$x:vst/injectValues:*[1]" />
    <xsl:call-template name="foldl">
      <xsl:with-param name="pFunc" select="$vInjectValues" />
      <xsl:with-param name="pA0"
      select="ext:node-set($vrtf-cp)/*" />
...
-->
  ...
  <xsl:template name="injectValues" match="injectValues:*">
    ...
    <!-- Current list -->
    <xsl:variable name="vextAllLists"
    select="$x:vst/extendAllLists:*[1]" />
  ...
  <xsl:template name="extLists" match="extendAllLists:*">
    ...
    <xsl:param name="arg1" select="/.." />
    <!-- Extending element -->
    <xsl:variable name="vappendToList"
    select="$x:vst/appendToList:*[1]" />
