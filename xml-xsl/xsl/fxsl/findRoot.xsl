...
xmlns:ext="http://exslt.org/common" xmlns:myIterator="f:myIterator"
xmlns:myBSIterator="f:myBSIterator"
xmlns:fxslFRBS-Cond="f:fxslFRBS-Cond" xmlns:x="f:x-findRoot.xsl"
exclude-result-prefixes="xsl x ext myIterator myBSIterator fxslFRBS-Cond">
  ...
  <xsl:template name="findRootBS">
      ...
      <xsl:otherwise>
        <xsl:if test="$vFX1 * $vFX2 &gt; 0">
          <xsl:message terminate="yes">
            <xsl:value-of select="concat('f(x1) = ', $vFX1, ' and f(x2) = ', $vFX2, ' must have different signs!')" />
          </xsl:message>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:variable name="vrtfX-Iter">
      ...
      <FX1>
        <xsl:value-of select="$vFX1" />
      </FX1>
      <X2>
        <xsl:value-of select="$pX2" />
      </X2>
      <FX2>
        <xsl:value-of select="$vFX2" />
      </FX2>
      <Eps>
        <xsl:value-of select="$pEps" />
      </Eps>
      <xsl:copy-of select="$pFun" />
    </xsl:variable>
    <xsl:variable name="rtftheRoot">
      <xsl:call-template name="iterUntil">
        ...
        <xsl:with-param name="pCond"
        select="$x:st/fxslFRBS-Cond:*[1]" />
        <xsl:with-param name="arg1"
        select="ext:node-set($vrtfX-Iter)/*" />
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template match="myBSIterator:*">
    ...
    <xsl:variable name="vFX1"
    select="number($arg1[name()='FX1'])" />
    <xsl:variable name="vFX2"
    select="number($arg1[name()='FX2'])" />
    <xsl:variable name="vEps"
    select="number($arg1[name()='Eps'])" />
    ...
    <X1>
      <xsl:value-of select="$vMid" />
    </X1>
    <FX1>
      <xsl:value-of select="$vFMid" />
    </FX1>
    <xsl:choose>
      <xsl:when test="$vFMid &gt; 0 and $vFX1 &gt; 0 or ($vFMid &lt; 0 and $vFX1 &lt; 0)">

        <X2>
          <xsl:value-of select="$vX2" />
        </X2>
        <FX2>
          <xsl:value-of select="$vFX2" />
        </FX2>
    ...
    <xsl:copy-of select="$arg1[name()='Eps']" />
    <!-- <Eps> -->
  ...
  <xsl:template match="fxslFRBS-Cond:*">
    ...
    <xsl:if test="-$vEps &lt;= $vResult and $vResult &lt;= $vEps">
    1</xsl:if>
  </xsl:template>
  <!-- ************************************************************* -->
  <!-- ********************* INTERNAL USE ONLY ********************* -->
