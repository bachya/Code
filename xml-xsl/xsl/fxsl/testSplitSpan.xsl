...
xmlns:mySpanController="mySpanController">
  ...
  <xsl:variable name="vSpanController"
  select="document('')/*/mySpanController:*[1]" />
  <xsl:output indent="yes" omit-xml-declaration="yes" />
  <xsl:template match="/">Split 4 
  <xsl:call-template name="split">
    <xsl:with-param name="pList" select="/*/*" />
    <xsl:with-param name="pN" select="4" />
  </xsl:call-template>Span &lt;= 3: 
  <xsl:call-template name="span">
    <xsl:with-param name="pList" select="/*/*" />
    <xsl:with-param name="pController" select="$vSpanController" />
  </xsl:call-template></xsl:template>
  <xsl:template name="mySpanController"
