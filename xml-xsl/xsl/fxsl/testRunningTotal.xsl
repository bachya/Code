...
xmlns:ext="http://exslt.org/common" xmlns:myAdd="f:myAdd"
xmlns:myMult="f:myMult">
    ...
    <html>
      <table border="2">
        ...
        <xsl:call-template name="myRunningTotal">
          <xsl:with-param name="vNodes" select="/booklist/book" />
        </xsl:call-template>
      </table>
    </html>
  </xsl:template>
  <xsl:template name="myRunningTotal">
    ...
    <xsl:variable name="vFunAdd"
    select="document('')/*/myAdd:*[1]" />
    <xsl:variable name="vFunMult"
    select="document('')/*/myMult:*[1]" />
    ...
    <xsl:for-each select="$vNodes">
      ...
      <tr>
        ...
        <td>
          <xsl:value-of select="$vRunningTotal [$vThisPosition]" />
        </td>
      </tr>
    </xsl:for-each>
  </xsl:template>
