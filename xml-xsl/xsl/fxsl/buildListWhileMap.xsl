...
exclude-result-prefixes="xsl ext">
  <xsl:template name="buildListWhileMap">
    ...
    <xsl:if test="not($pController)">
      <xsl:message terminate="yes">[buildListWhileMap]Error: No
      pController specified: would cause infinite
      processing.</xsl:message>
    </xsl:if>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:apply-templates select="$pMap">
              ...
              select="ext:node-set($vNewBaseListElement)/*" />
              <xsl:with-param name="pList" select="$pList" />
            </xsl:apply-templates>
          </xsl:otherwise>
        </xsl:choose>
    ...
    select="ext:node-set($newList)/*" />
    <xsl:variable name="vResultBaseList"
    select="ext:node-set($newRTFBaseList)/*" />
