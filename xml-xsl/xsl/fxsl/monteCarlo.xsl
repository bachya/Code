...
xmlns:ext="http://exslt.org/common" xmlns:x="f:fxsl-monteCarlo"
xmlns:genTest="f:genTest" xmlns:rFloat="f:rFloat"
exclude-result-prefixes="xsl ext x genTest rFloat">
  ...
  <xsl:template name="monteCarlo">
    ...
    <xsl:variable name="vGenAndTest" select="$x:st/genTest:*[1]" />
    <xsl:variable name="vRFloat" select="$x:st/rFloat:*[1]" />
    <xsl:variable name="vIntRange" select="$modulus - 1" />
    ...
    <xsl:variable name="vrtf-rndFloatX">
      <xsl:call-template name="curry">
        ...
        <xsl:with-param name="pFun" select="$vRFloat" />
        <xsl:with-param name="arg1"
        select="($arg4 - $arg3) div $vIntRange" />
        <xsl:with-param name="arg2" select="$arg3" />
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:variable name="vrtf-rndFloatY">
      <xsl:call-template name="curry">
        ...
        <xsl:with-param name="pFun" select="$vRFloat" />
        <xsl:with-param name="arg1"
        select="($arg6 - $arg5) div $vIntRange" />
        <xsl:with-param name="arg2" select="$arg5" />
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:value-of select="$vnHits div $arg1 * $vSpace" />
  </xsl:template>
  <xsl:template match="genTest:*">
    ...
    <xsl:variable name="vCount" select="$arg1[1]" />
    <xsl:variable name="vSeed" select="$arg1[2]" />
    <xsl:variable name="vNextSeed">
      <xsl:call-template name="randNext">
        <xsl:with-param name="arg1" select="$vSeed" />
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="v-rndX">
      <xsl:apply-templates select="$arg2">
        <xsl:with-param name="arg3" select="$vSeed" />
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:variable name="vF-rndX">
      <xsl:apply-templates select="$arg4">
        <xsl:with-param name="arg1" select="$v-rndX" />
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:variable name="v-rndY">
      <xsl:apply-templates select="$arg3">
        <xsl:with-param name="arg3" select="$vNextSeed" />
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:choose>
