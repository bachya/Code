...
xmlns:ext="http://exslt.org/common">
  ...
  select="ext:node-set($vrtfStore0)/*" />
  <!-- Get A, B, C from the initial store: must be 0-s -->
  vStore0: A='
  <xsl:apply-templates select="$vStore0/*[local-name() = 'getValue']">
  ...
  select="ext:node-set($vrtfStore1)/*" />
  <!-- Get A, B, C from the store1: A is 1, the rest must be 0-s -->
  vStore1: A='
  <xsl:apply-templates select="$vStore1/*[local-name() = 'getValue']">
  ...
  select="ext:node-set($vrtfStore2)/*" />
  <!-- Get A, B, C from the store2: A is 1, B is 2, the rest must be 0-s -->
  vStore2: A='
  <xsl:apply-templates select="$vStore2/*[local-name() = 'getValue']">
  ...
  select="ext:node-set($vrtfStore3)/*" />
  <!-- Get A, B, C from the store3: A is 1, B is 2, C is 3, the rest must be 0-s -->
  vStore3: A='
  <xsl:apply-templates select="$vStore3/*[local-name() = 'getValue']">
