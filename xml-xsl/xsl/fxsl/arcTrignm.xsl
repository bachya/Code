...
xmlns:ext="http://exslt.org/common" xmlns:mySinFun="f:mySinFun"
xmlns:mySinFunPrim="f:mySinFunPrim" xmlns:myTanFun="f:myTanFun"
xmlns:myTanFunPrim="f:myTanFunPrim" xmlns:myCotFun="f:myCotFun"
xmlns:myCotFunPrim="f:myCotFunPrim" xmlns:atrigSin="f:atrigSin"
xmlns:atrigCos="f:atrigCos" xmlns:atrigTan="f:atrigTan"
xmlns:atrigCot="f:atrigCot" xmlns:atrigSec="f:atrigSec"
xmlns:atrigCsc="f:atrigCsc" xmlns:x="f:arcTrignm.xsl"
exclude-result-prefixes="xsl x ext mySinFun mySinFunPrim myTanFun myTanFunPrim myCotFun myCotFunPrim atrigSin atrigCos atrigTan atrigCot atrigSec atrigCsc">
...
-->
  ...
  <xsl:template name="_atrigWrapper">
    ...
    <xsl:value-of select="(($pUnit = 'rad') * $vradResult) + ((not($pUnit = 'rad')) * ($vradResult * $rad2degs))" />
  </xsl:template>
  <xsl:template name="_arcsin" match="atrigSin:*">
    <xsl:param name="pX" />
    <xsl:param name="pEps" select="0.0000001" />
    <xsl:if test="not(-1 &lt;= $pX and $pX &lt;= 1)">
      <xsl:message terminate="yes">[Error]: arcSine argument must
      be within the interval [-1, 1]</xsl:message>
    </xsl:if>
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:call-template name="findRootNR">
          ...
          select="ext:node-set($vrtf-curriedSinFun)/*" />
          <xsl:with-param name="pFunPrim"
          select="ext:node-set($vrtf-curriedCoSFun)/*" />
  ...
  <xsl:template name="_arctan" match="atrigTan:*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="vTanFun"
        select="$x:vst/myTanFun:*[1]" />
        <xsl:variable name="vTanPrFun"
        select="$x:vst/myTanFunPrim:*[1]" />
        ...
        <xsl:variable name="vrawRoot">
          <xsl:call-template name="findRootNR">
            <xsl:with-param name="pFun"
            select="ext:node-set($vrtf-curriedTanFun)/*" />
            <xsl:with-param name="pFunPrim"
            select="ext:node-set($vrtf-curriedTanPrFun)/*" />
  ...
  <xsl:template name="_arccot" match="atrigCot:*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="vCotFun"
        select="$x:vst/myCotFun:*[1]" />
        <xsl:variable name="vCotPrFun"
        select="$x:vst/myCotFunPrim:*[1]" />
        ...
        <xsl:variable name="vrawRoot">
          <xsl:call-template name="findRootNR">
            <xsl:with-param name="pFun"
            select="ext:node-set($vrtf-curriedCotFun)/*" />
            <xsl:with-param name="pFunPrim"
            select="ext:node-set($vrtf-curriedCotPrFun)/*" />
            <xsl:with-param name="pX0" select="3" />
            <xsl:with-param name="pEps" select="0.0000001" />
          </xsl:call-template>
        </xsl:variable>
  ...
  <xsl:template name="_arcsec" match="atrigSec:*">
    ...
    <xsl:param name="pEps" select="0.0000001" />
    <xsl:if test="$pX = 0">
      <xsl:message terminate="yes">[Error]arcsec: The argument
      cannot be 0.</xsl:message>
    </xsl:if>
  ...
  <xsl:template name="_arccsc" match="atrigCsc:*">
    ...
    <xsl:param name="pEps" select="0.0000001" />
    <xsl:if test="$pX = 0">
      <xsl:message terminate="yes">[Error]arccsc: The argument
      cannot be 0.</xsl:message>
    </xsl:if>
    <xsl:call-template name="_arcsin">
      <xsl:with-param name="pX" select="1 div $pX" />
