...
exclude-result-prefixes="f gd" version="1.0">
  ...
  <xsl:template match="f:entry[not(gd:recurrence)]">
    ...
    select="gd:when/@startTime" />
    <xsl:variable name="startDate"
    select="substring($startDateTime, 0, 11)" />
    <xsl:variable name="startTime"
    select="substring-before(substring-after($startDateTime, 'T'), '.')" />
    <xsl:variable name="endDateTime" select="gd:when/@endTime" />
    <xsl:variable name="endDate"
    select="substring($endDateTime, 0, 11)" />
    <xsl:variable name="endTime"
    select="substring-before(substring-after($endDateTime, 'T'), '.')" />
    <Event>
      <EventTitle>
  ...
  <xsl:template match="f:entry[gd:recurrence]" />
</xsl:stylesheet>
