...
exclude-result-prefixes="f pGenerator DropWhileController 
                         MyRepeatGenerator MyRepeatableFunction"
>

  <xsl:import href="../f/take.xsl"/> 
  <xsl:import href="../f/drop.xsl"/>
  <xsl:import href="../f/dropWhile.xsl"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:copy-of select="$vTake8/*"/>
   
    <xsl:text>&#xA;</xsl:text>
    <xsl:text>&#xA;</xsl:text>
      
    <xsl:call-template name="take">
      <xsl:with-param name="pList" select="$vTake8/*"/>
  ...
  <xsl:template name="myRepeater" match="*[namespace-uri()='MyRepeatGenerator']"
   mode="f:FXSL">
     ...
     <xsl:choose>
	     ...
	     <xsl:otherwise>
	       <xsl:apply-templates select="$pParamGenerator[1]" mode="f:FXSL">
