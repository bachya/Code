...
xmlns:roundLog2="roundLog2" exclude-result-prefixes="roundLog2">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:element name="{name($pList[1])}">
          <xsl:value-of select="($pList[2] * $v2__N - $pList[1]) div ($v2__N - 1)" />
        </xsl:element>
        ...
        <xsl:variable name="v1">
          <xsl:choose>
            <xsl:when test="not($pList[2] = $pList[3])">
              <xsl:value-of select="($pList[1] - $pList[3]) div ($pList[2] - $pList[3]) - 1" />
            </xsl:when>
