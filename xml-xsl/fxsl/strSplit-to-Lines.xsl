...
exclude-result-prefixes="xsl ext str-split2lines-func">
  ...
  <xsl:template name="str-split-to-lines">
    ...
    <xsl:variable name="vrtfParams">
      <delimiters>
        <xsl:value-of select="$pDelimiters" />
      </delimiters>
      <lineLength>
        <xsl:copy-of select="$pLineLength" />
      </lineLength>
    </xsl:variable>
  ...
  <xsl:template match="str-split2lines-func:*">
    ...
    <xsl:choose>
      <xsl:when test="contains($arg1/*[1], $arg2)">
        <xsl:if test="string($arg1/word)">
          <xsl:call-template name="fillLine">
            ...
            <xsl:with-param name="pLineLength"
            select="$arg1/*[2]" />
          </xsl:call-template>
        </xsl:if>
      </xsl:when>
      <xsl:otherwise>
        ...
        <word>
          <xsl:value-of select="concat($arg1/word, $arg2)" />
        </word>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="fillLine">
    ...
    <xsl:variable name="vnWordsInLine"
    select="count($pLine/word)" />
