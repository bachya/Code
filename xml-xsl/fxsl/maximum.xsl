...
exclude-result-prefixes="xsl ext maximum-own-compare maximum-pick-bigger">
  ...
  <xsl:template name="maximum">
    ...
    <xsl:if test="$pList">
      ...
      <xsl:variable name="vFuncList">
        <xsl:copy-of select="$vFoldFun" />
        <!-- Pick Bigger -->
        <xsl:copy-of select="$vCMPFun" />
        <!-- Compare -->
      </xsl:variable>
      <xsl:variable name="vrtfResult">
        <xsl:call-template name="foldl">
          <xsl:with-param name="pFunc"
          select="ext:node-set($vFuncList)/*" />
  ...
  <xsl:template name="pickBigger" match="maximum-pick-bigger:*">
    ...
    <xsl:copy-of select="$arg1[$vIsGreater = 1] | $arg2[not($vIsGreater = 1)]" />
  </xsl:template>
  <xsl:template name="isGreaterDefault"
  match="maximum-own-compare:*">
    <xsl:param name="arg1" />
    <xsl:param name="arg2" />
    <xsl:choose>
