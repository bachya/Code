...
exclude-result-prefixes="xsl ext pGenerator pController IntervalParams easy-integrate">
  ...
  <xsl:template name="partialSumsList">
    ...
    select="ext:node-set($vrtfvIntervalParams)/*" />
    <xsl:variable name="vrtfResultIntervalList">
      <xsl:call-template name="buildListWhileMap">
        ...
        <xsl:with-param name="pMap"
        select="$vmyEasyIntegrateMap" />
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="multiIntegrate">
    ...
    <xsl:call-template name="foldl">
      ...
      select="$pList[position() &gt; 1 and position() &lt; last()]" />
  ...
  match="*[namespace-uri()='easy-integrate']">
    ...
    <!-- node -->
    <xsl:value-of select="$arg1 + (($arg2 + $arg2/following-sibling::*[1]) div 2 ) * $arg2/../*[1]" />
  </xsl:template>
</xsl:stylesheet>
