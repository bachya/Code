...
xmlns:myIncrement="f:myIncrement" xmlns:x="f:fxslRandomList.xsl"
...
-->
  <xsl:template name="_randScale">
    ...
    <!--pStart-->
    <xsl:param name="arg3" select="1" />
    <!--pEnd  -->
    <xsl:param name="arg1" />
    <!--pN    -->
    <xsl:value-of select="floor( ($arg3 - $arg2 + 1) div ($modulus - 1) * $arg1 + $arg2 )" />
  </xsl:template>
  <!--
      Template: _dScale                                    
    ...
    <xsl:if test="$pRandSeq">
      <el>
        <xsl:call-template name="_randScale">
          ...
          <!--s-->
          <xsl:with-param name="arg3" select="count($pRandSeq)" />
          <!--end-->
...
-->
  ...
  <xsl:template name="_permutationFromRecursiveIndex">
    ...
    <xsl:if test="not(count($pList) = count($pRecIndex))">
      ...
      Error[permutationFromRecursiveIndex]: The two lists are not
      the same length!</xsl:message>
    </xsl:if>
  ...
  <xsl:template name="_permRecIdx">
    <xsl:param name="pList" select="/.." />
    <xsl:param name="pRecIndex" select="/.." />
    <xsl:variable name="vIndex" select="number($pRecIndex[1])" />
    <xsl:if test="$pList">
      <el>
        <xsl:value-of select="$pList[position() = $vIndex]" />
      </el>
      <xsl:call-template name="_permRecIdx">
  ...
  <xsl:template name="_mapFromRandIndex">
    ...
    <xsl:param name="pRecIndex" select="/.." />
    <xsl:if test="not(count($pList) = count($pRecIndex))">
      <xsl:message terminate="yes">Error[mapFromRandIndex]: The two
      lists are not the same length!</xsl:message>
    </xsl:if>
