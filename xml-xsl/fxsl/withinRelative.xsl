...
exclude-result-prefixes="xsl ext MyWithinRelEpsController">
  ...
  <xsl:variable name="vMyWithinRelEpsController"
  select="document('')/*/MyWithinRelEpsController:*[1]" />
  ...
  match="*[namespace-uri()='MyWithinRelEpsController']">
    ...
    <xsl:choose>
      ...
      <xsl:when test="count($pList) &gt;= 2">
        ...
        <xsl:variable name="lastProd"
        select="$pParams * $pList[last()]" />
        <xsl:if test="not($lastDiff &lt;= $lastProd and $lastDiff &gt;= (0 - $lastProd))">
        1</xsl:if>
      </xsl:when>
