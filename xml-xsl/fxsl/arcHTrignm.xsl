...
xmlns:ext="http://exslt.org/common" xmlns:myhSinFun="f:myhSinFun"
xmlns:myhSinFunPrim="f:myhSinFunPrim" xmlns:myhTanFun="f:myhTanFun"
xmlns:myhTanFunPrim="f:myhTanFunPrim" xmlns:myhCotFun="f:myhCotFun"
xmlns:myhCotFunPrim="f:myhCotFunPrim" xmlns:atrighSin="f:atrighSin"
xmlns:atrighCos="f:atrighCos" xmlns:atrighTan="f:atrighTan"
xmlns:atrighCot="f:atrighCot" xmlns:atrighSec="f:atrighSec"
xmlns:atrighCsc="f:atrighCsc" xmlns:x="f:arcHTrignm.xsl"
exclude-result-prefixes="xsl x ext myhSinFun myhSinFunPrim myhTanFun myhTanFunPrim myhCotFun myhCotFunPrim atrighSin atrighCos atrighTan atrighCot atrighSec atrighCsc">
...
-->
  ...
  <xsl:template name="_archsin" match="atrighSin:*">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="vhSinFun"
        select="$x:vst/myhSinFun:*[1]" />
        <xsl:variable name="vhCoSFun"
        select="$x:vst/myhSinFunPrim:*[1]" />
        ...
        <xsl:call-template name="findRootNR">
          ...
          select="ext:node-set($vrtf-curriedhSinFun)/*" />
          <xsl:with-param name="pFunPrim"
          select="ext:node-set($vrtf-curriedhCoSFun)/*" />
...
-->
    <xsl:if test="$pX &gt;= 1 or $pX &lt;= -1">
      <xsl:message terminate="yes">[Error]archtan: X is outside the
      interval (-1, 1)</xsl:message>
    </xsl:if>
  ...
  <xsl:template name="_archcot" match="atrighCot:*">
    ...
    <xsl:if test="$pX &gt;= -1 and $pX &lt;= 1">
      <xsl:message terminate="yes">[Error]archcot: X is must be
      outside the interval [-1, 1]</xsl:message>
    </xsl:if>
  ...
  <xsl:template name="_archsec" match="atrighSec:*">
    ...
    <xsl:if test="$pX &lt;= 0 or $pX &gt; 1">
      <xsl:message terminate="yes">[Error]archcot: X is must be in
      the interval (0, 1]</xsl:message>
    </xsl:if>
    <xsl:call-template name="_archcos">
      <xsl:with-param name="pX" select="1 div $pX" />
      <xsl:with-param name="pEps" select="$pEps" />
