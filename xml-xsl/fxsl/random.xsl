...
xmlns:ext="http://exslt.org/common" xmlns:randScale="f:randScale"
  ...
  <xsl:template name="randomSequence">
    ...
    <xsl:variable name="vrtfUnscaledSeq">
      <xsl:call-template name="scanIter">
        <xsl:with-param name="arg1" select="$pLength - 1" />
        <!-- n -->
        <xsl:with-param name="arg2" select="$vFunRandNext" />
        <!-- f -->
        <xsl:with-param name="arg3" select="$pSeed" />
        <!-- x -->
      </xsl:call-template>
    </xsl:variable>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:call-template name="scaleSequence">
          ...
          select="ext:node-set($vrtfUnscaledSeq)/*" />
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
...
-->
  ...
  <xsl:template name="dist-randomSequence">
    ...
    <xsl:call-template name="map">
      ...
      select="ext:node-set($vrtfDistFunction)/*" />
