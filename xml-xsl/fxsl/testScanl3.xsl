...
xmlns:ext="http://exslt.org/common" xmlns:auxSums="f:auxSums"
...
exclude-result-prefixes="xsl ext auxSums zeroSums">
  ...
  <xsl:template match="/">
    <xsl:variable name="vZeroSums"
    select="document('')/*/zeroSums:*[1]" />
    <xsl:variable name="vfAuxSums"
    select="document('')/*/auxSums:*[1]" />
  ...
  <xsl:template match="auxSums:*">
    ...
    <s>
      <xsl:value-of select="$pArg1/s + $pArg2" />
    </s>
    <sq>
      <xsl:value-of select="$pArg1/sq + $pArg2 * $pArg2" />
    </sq>
  </xsl:template>
</xsl:stylesheet>
