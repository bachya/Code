...
xmlns:ext="http://exslt.org/common" xmlns:mySquare="f:mySquare"
...
exclude-result-prefixes="xsl ext mySquare myDouble">
  ...
  </xsl:variable>Random Recursive Index (dScale (randomSequence
  100)): 
  <xsl:call-template name="_dScale">
    <xsl:with-param name="pRandSeq"
    select="ext:node-set($vrtfRands)/*" />
  </xsl:call-template>Random Recursive Index 10: 
  ...
  select="document('')/*/myDouble:*[1]" />randomMap (*2) [1..10]
  seed: 
  <xsl:call-template name="randomMap">
    <xsl:with-param name="pFun" select="$vFunDouble" />
    <xsl:with-param name="pList" select="/*/*" />
  </xsl:call-template>randListIndex [1..10] seed: 
