...
exclude-result-prefixes="ext curryPartialApplicator">
  <xsl:template name="curry">
    ...
    <xsl:if test="$pNargs &lt; 2">
      <xsl:message terminate="yes">[curry]Error: pNargs (number of
      arguments) of a fn to be curried must be at least
      2</xsl:message>
    </xsl:if>
    <xsl:if test="$pNargs &gt; 10">
      <xsl:message terminate="yes">[curry]Error: pNargs (number of
      arguments) of a fn to be curried must not exceed
      10</xsl:message>
    </xsl:if>
    ...
    <xsl:variable name="vrtfCurriedNoArgs">
      <curryPartialApplicator:curryPartialApplicator>
        ...
        <curryNumargs>
          <xsl:value-of select="$pNargs" />
        </curryNumargs>
        <curryStrip>
          <xsl:value-of select="$pStripAuxNamespace" />
        </curryStrip>
      </curryPartialApplicator:curryPartialApplicator>
    </xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="not($args) and not($arg1 or $arg2 or $arg3 or $arg4 or $arg5 or $arg6 or $arg7 or $arg8 or $arg9 or $arg10 )">
  ...
  <xsl:template match="curryPartialApplicator:*">
    ...
    <xsl:param name="arg9" select="/.." />
    <xsl:param name="arg10" select="/.." />
    <xsl:if test="not($args) and not($arg1 or $arg2 or $arg3 or $arg4 or $arg5 or $arg6 or $arg7 or $arg8 or $arg9 or $arg10 )">

      <xsl:message terminate="yes">[currying]Error: No arguments
    ...
    select="ext:node-set($vrtvArgs)/*" />
    <xsl:if test="count($vArgs) &gt; curryNumargs">
      <xsl:message>
        <xsl:value-of select="concat('[currying]Error: The number of actual arguments supplied is ', count($vArgs), ' -- bigger than the number of arguments of this function:', curryNumargs )" />
      </xsl:message>
    </xsl:if>
    <xsl:if test="$vArgs = curryArgs/*">
      <xsl:message>[currying]Error: Same argument(s) fixed
      twice</xsl:message>
    </xsl:if>
    <xsl:for-each select="$vArgs[not(position()=last())]">
      <xsl:variable name="vThisPosition" select="position()" />
      <xsl:if test=". = $vArgs[position() &gt; $vThisPosition and name() = name(current()) ]">

        <xsl:message>[currying]Error: Same argument(s) specified
        twice</xsl:message>
      </xsl:if>
    </xsl:for-each>
    <!-- Normal  Processing -->
    ...
    select="ext:node-set($vrtfNewFun)/*" />
    <xsl:choose>
