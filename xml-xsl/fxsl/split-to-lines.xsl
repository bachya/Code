...
xmlns:ext="http://exslt.org/common" xmlns:pGenerator="f:pGenerator"
...
exclude-result-prefixes="xsl ext pGenerator pController">
  ...
  <xsl:template match="/">
    ...
    select="ext:node-set($prtfSplitParams)" />
    <xsl:variable name="vNumLines"
    select="ceiling(string-length($pSplitParams/text) div $pSplitParams/lineLength )" />
