...
exclude-result-prefixes="xsl ext pGenerator DropWhileController MyRepeatGenerator MyRepeatableFunction">
  ...
  <xsl:variable name="vDropWhileController"
  select="document('')/*/DropWhileController:*[1]" />
  ...
  <xsl:template name="myDWController"
  match="*[namespace-uri()='DropWhileController']">
    <xsl:param name="pList" select="/.." />
    <xsl:param name="pParams" />
